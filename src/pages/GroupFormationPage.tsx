        ...
        <div className="card">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-900">
              {mode === 'create' ? 'Create Your Group' : 'Join Existing Group'}
            </h1>
            <p className="text-gray-600 mt-2">
              {mode === 'create' 
                ? "You'll be the Group Representative and submit the ballot entry for everyone"
                : "Join an existing group that has space for more members"
              }
            </p>
          </div>
            ...
            <form onSubmit={handleCreateGroup} className="space-y-6">
              ...
              <div>
                ...
                {(selectedParticipants.length + members.length) < 2 && (
                  <div className="mt-4">
                    <label htmlFor="newMember" className="block text-sm font-medium text-gray-700 mb-2">
                      Or Add by Email (Groups: 1-3 people)
                    </label>
                ...
                <div className="mt-3 text-sm text-gray-600">
                  <p>Group Size: {selectedParticipants.length + members.length + 1}/3 members</p>
                  <p className="mt-1">⚠️ All members must be registered participants</p>
                </div>
              </div>
            ...
            <form onSubmit={handleJoinGroup} className="space-y-6">
              ...
              <div>
                ...
                {joinableGroups.length === 0 ? (
                  <div className="text-center py-8 text-gray-500">
                    <div className="text-4xl mb-4">🔍</div>
                    <p className="text-lg">No available groups to join</p>
                    <p className="text-sm mt-2">Try creating your own group instead</p>
                  </div>
                ) : (
                  <>
                      ...
                      {joinableGroups.map((group) => (
                        <div key={group.id} className={`p-4 border rounded-lg cursor-pointer transition-colors ${
                          ...
                          <div className="text-sm text-gray-600">
                            <p>👑 Representative: {group.representative}</p>
                            <p>📅 Created: {new Date(group.createdAt).toLocaleDateString()}</p>
                            {group.members.length > 1 && (
                              <p>👥 Members: {group.members.filter(m => m !== group.representative).join(', ')}</p>
                            )}
                          </div>
                          <div className="mt-2 flex items-center text-xs text-green-600">
                            ✓ {3 - group.members.length} space{3 - group.members.length !== 1 ? 's' : ''} available
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      <p className="text-sm text-blue-700">
                        💡 <strong>Tip:</strong> Click on a group to select it, then click "Join Selected Group" to become a member.
                      </p>
                    </div>
                  </>
                )}
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="flex-1 btn-primary"
                >
                  {isSubmitting ? 'Joining Group...' : 'Join Selected Group'}
                </button>
                <button
                  type="button"
                  onClick={() => onNavigate('landing')}
